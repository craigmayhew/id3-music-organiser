...
fn main() -> std::io::Result<()> {
    ...
    */
    ...
    for entry in WalkDir::new("./unsorted")
        .into_iter()
        .filter_map(|e| e.ok())
    {
        ...
        if let Ok(_res) = id3::Tag::is_candidate(file) {
            if let Ok(_res) = id3::Tag::read_from_path(path) {
                ...
                let destination_path_with_file_name: String = destination_path_with_file_name(
                    path,
                    &destination,
                    &artist,
                    &album_name,
                ...
                println!(
                    "{}{}\n",
                    "  COPYING FILE to ".cyan(),
                    destination_path_with_file_name.cyan()
                );
    ...
    println!(
        "----------------------------------\n{}{}",
        "  FILES COPIED ".green().bold(),
        file_mv_counter.to_string().bold()
    );
    println!(
        "{}{}",
        "  FILES SKIPPED ".green().bold(),
        file_skipped_counter.to_string().bold()
    );
    Ok(())
}
...
fn destination_path_with_file_name(
    path: &Path,
...
mod destination_path_with_file_name {
    ...
    fn check_creates_file() {
        ...
        assert_eq!(
            destination_path_with_file_name(path, "unittest-sorted", "artist", "album", "title"),
            "unittest-sorted/artist/album/title.mp3".to_owned()
        );
...
fn title(tag_title: Option<&str>) -> std::string::String {
    ...
    let title = RE_GOOD_CHARS_ONLY
        .replace_all(tag_title.unwrap(), "")
        .to_string();
    title
}
...
mod album {
    ...
    fn album_basic() {
        ...
        assert_eq!(
            album(option_mock_id3_album, option_mock_id3_album_artist),
            str_mock_album
        );
    }
    #[test]
